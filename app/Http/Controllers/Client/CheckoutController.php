...
class CheckoutController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Nếu đã có đơn pending → chuyển luôn sang trang thanh toán
        ...
        $status = $request->shipping_method === 'cod' ? 'processing' : 'pending';
        ...
        $order = Order::create([
            ...
            'product_image_url' => $variant->product->image ?? null,
        ...
        foreach ($cart->items as $item) {
            ...
            $order->items()->create([
                ...
                'product_image_url' => 'storage/' . $variant->product->image,
            ]);
            if ($variant->stock_quantity >= $item->quantity) {
                $variant->stock_quantity -= $item->quantity;
                $variant->save();
            } else {
                return redirect()->route('cart.index')->with('error', 'Sản phẩm "' . $variant->product->name . '" không đủ hàng.');
            }
        }
    ...
    public function paymentStore(Request $request)
    {
        ...
        if ($request->payment_method === 'bank_transfer') {
            // ✅ Gửi khách đến trang cảm ơn kèm hướng dẫn chuyển khoản
            return redirect()->route('checkout.thankYou', $order->id)->with('bank_transfer', true);
        }
