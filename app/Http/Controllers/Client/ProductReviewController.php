...
{   
    ...
    // Xử lý ảnh: ép $review->images thành array an toàn
    $currentImages = is_array($review->images) 
        ? $review->images 
        : json_decode($review->images, true) ?? [];

    $images = $this->handleImageUpload($request, $currentImages);

    $review->update([
        'rating' => $validated['rating'],
        'comment' => $validated['comment'] ?? null,
        'images' => $images,   // luôn là array
        'is_edited' => true,
    ...
    return redirect()
        ->route('client.orders.show', $review->order_id)
    ...
    if ($request->has('remove_images')) {
        foreach ($request->remove_images as $imageToRemove) {
            if (in_array($imageToRemove, $images)) {
                ...
                $images = array_filter($images, fn($image) => $image !== $imageToRemove);
            }
        }
