...
@section('container-fluid')
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div class="card-tools">
                            <span class="badge bg-secondary">{{ $categories->total() }} mục</span>
                        </div>
                    </div>
                        ...
                        @if ($categories->count() > 0)
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <small class="text-muted">
                                                        ...
                                                        <br>
                                                        <i class="fas fa-clock me-1"></i>
                                                        {{ $category->deleted_at->format('H:i:s') }}
                                                    </small>
                                                </td>
                                                <td class="text-center">
                                                    <div class="btn-group" role="group" aria-label="Actions">
                                                        ...
                                                        <form action="{{ route('categories.restore', $category->id) }}"
                                                            method="POST" class="d-inline-block restore-form">
                                                            @csrf
                                                            <button type="button"
                                                                class="btn btn-sm btn-success restore-btn"
                                                                data-bs-toggle="tooltip" title="Khôi phục danh mục">
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-3">
                                <div class="text-muted">
                                    ...
                                    trong tổng số {{ $categories->total() }} mục
                                </div>
                                <div>
                                    {{ $categories->links() }}
                                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form action="{{ route('categories.restoreAll') }}" method="POST" class="mb-3">
                        @csrf
                        <button type="button" class="btn btn-success w-100 restore-all-btn">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.restore-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    Swal.fire({
                        ...
                        text: "Danh mục sẽ được khôi phục!",
                        icon: 'warning',
            ...
            document.querySelectorAll('.restore-all-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    Swal.fire({
                        title: 'Bạn có chắc chắn?',
                        text: "Tất cả danh mục đã xóa sẽ được khôi phục!",
                        icon: 'warning',
                        showCancelButton: true,
                        confirmButtonColor: '#28a745',
                        cancelButtonColor: '#6c757d',
                        confirmButtonText: 'Có, khôi phục tất cả!',
                        cancelButtonText: 'Hủy bỏ'
                    }).then((result) => {
                        if (result.isConfirmed) {
                            this.closest('form').submit();
