                    ...
                    @if($variant_attributes->count() > 0)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="btn-group" role="group">
                                                <form action="{{ route('attributes.restore', $attribute->id) }}" method="POST" class="d-inline-block">
                                                    @csrf
                                                    <button type="submit" onclick="return confirm('Khôi phục thuộc tính này?')" class="btn btn-sm btn-success">
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-3">
                            <div class="text-muted">
                                Hiển thị {{ $variant_attributes->firstItem() }} - {{ $variant_attributes->lastItem() }} trong tổng số {{ $variant_attributes->total() }} mục
                            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('attributes.restoreAll') }}" method="POST" class="mb-3">
                    @csrf
                    <button type="submit" class="btn btn-success w-100" onclick="return confirm('Khôi phục tất cả thuộc tính đã xóa?')">
